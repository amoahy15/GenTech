...
const AnnotationComments = ({ comments, clickCoordinates, onAddCommentClick }) => {
  ...
  const handleAddAnnotationClick = () => {
    setShowCoordinates(true); // Set showCoordinates to true immediately after clicking "+"
    if (showCoordinates) {
      console.log("Clicked again to annotate");
      onAddCommentClick(annotationText);
      setAnnotationText(""); // Clear the input field after adding the annotation
    }
  };
  ...
  const handleFormSubmit = (event) => {
    ...
    handleAddAnnotationClick(); // Call the function to handle adding the annotation
  };
